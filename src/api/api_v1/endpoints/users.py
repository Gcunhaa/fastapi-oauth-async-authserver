...
async def read_users(
    skip: int = 0, limit: int = 100, user_data: UserData = Depends(UserData)
) -> Any:
    """
    Retrieve users.
    """
...
@router.post("/")
async def create_user(request: UserCreateIn) -> Any:
    """
    Create user
    """
    user: ORMUser = await ORMUser.query.where(
        ORMUser.email == request.email
    ).gino.first()
...
async def read_user(id: int, user_data: UserData = Depends()) -> Any:
    """
    Retrieve user by id
    """
...
async def update_user(
    id: int, request: UserUpdateIn, user_data: UserData = Depends()
) -> Any:
    """
    Update user
    """
