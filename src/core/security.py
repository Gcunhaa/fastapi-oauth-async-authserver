...
with open("core/keys/private.pem", "rb") as pemfile:
    private_key = serialization.load_pem_private_key(
        data=pemfile.read(), password=b"vertrigo", backend=default_backend()
    )
...
with open("core/keys/public.pem", "rb") as pemfile:
    public_key = serialization.load_pem_public_key(
        data=pemfile.read(), backend=default_backend()
    )
...
async def verify_auth_form(
    form_data: AuthLoginForm = Depends(AuthLoginForm),
) -> AuthLoginForm:
    ...
    if form_data.grant_type == "password":
        if (
            ...
            or form_data.refresh_token != None
        ):
            raise HTTPException(status_code=400, detail={"error": "invalid_request"})
...
async def create_refresh_token(user: User) -> str:
    ...
    refresh_token: RefreshToken = RefreshToken(
        ...
        + timedelta(days=settings.REFRESH_TOKEN_EXPIRATION_TIME),
        token=token,
    )
...
async def verify_refresh_token(refresh_token: str) -> bool:
    founding_token: RefreshToken = await RefreshToken.query.where(
...
async def create_email_confirmation_token(user: PreUser) -> str:
    ...
    confirm_email_token: ConfirmEmailToken = ConfirmEmailToken(
        pre_user_id=user.id,
        valid_until=datetime.utcnow()
        + timedelta(minutes=settings.EMAIL_CONFIRMATION_TOKEN_EXPIRATION_TIME),
        token=token,
    )
...
async def create_password_change_token(user: User) -> str:
    ...
    change_password_token: ChangePasswordToken = ChangePasswordToken(
        user_id=user.id,
        valid_until=datetime.utcnow()
        + timedelta(minutes=settings.PASSWORD_CHANGE_TOKEN_EXPIRATION_TIME),
        token=token,
    )
